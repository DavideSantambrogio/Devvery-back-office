...
class UpdateUserDetailRequest extends FormRequest
{
    ...
    {
        return [
            'vat_number' => ['required', 'string', 'min:6', 'max:20', Rule::unique('user_details')->ignore($this->userDetail)],
            'phone' => ['required', 'string','min:10', 'max:15', Rule::unique('user_details')->ignore($this->userDetail)],
